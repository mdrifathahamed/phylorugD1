...
to black (clade present). Nodes supported unanimously by all analyses
are shown as a solid black dot -- maximally clean. Nodes where analyses
...
After translation the raw codes no longer exist and `root()` cannot
find them.
...
2x2 grid. The layout is the same at every node across the entire tree,
...
Multidimensional Scaling (MDS). Trees close together have similar
topologies. Trees far apart disagree substantially.
...
coordinates from the active plot environment and draws a 2x2 rug grid
at each uncertain node. Nodes where all four analyses agree are shown
as a solid black dot.
...
and co-supervised by Salvador Arias Becerra, University of Helsinki.
